...
def Forward_2D(sigma,epsilon,mu,para):
    ...
    for index,data_position in enumerate(zip(para.source_site,para.receiver_site)):
        res=pool.apply_async(Forward2D,args=(para.xl,para.zl,para.Freq,para.k_max,sigma.copy(),epsilon.copy(),mu.copy(),para.dx,para.dz,para.dt,data_position[0],data_position[1],index))
        res_l.append(res)
    pool.close()
    pool.join()
    for res in res_l:
...
def Forward2D(xl,zl,Freq,k_max,sigma,epsilon,mu,dx,dz,dt,value_source,value_receiver,index):
